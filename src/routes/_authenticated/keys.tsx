...
function KeysPage() {
  ...
  const create = useMutation({
    ...
    onError: (e: unknown) => toast.error(e instanceof Error ? e.message : "Could not mint key"),
  });
  ...
  const active = keyRows.filter((k) => !k.revoked_at);
  const revoked = keyRows.filter((k) => k.revoked_at);
  ...
  return (
    <main className="mx-auto max-w-5xl px-6">
      <header className="flex items-baseline justify-between border-b border-border/40 pb-6">
        <div>
          ...
          <h1 className="mt-3 font-display text-[52px] leading-none tracking-tight sm:text-[72px]">
            {active.length}
            <span className="ml-3 align-top font-sans text-[13px] uppercase tracking-[0.14em] text-muted-foreground">
              active
            </span>
          </h1>
          <p className="mt-4 max-w-xl text-[13.5px] text-muted-foreground">
            Each key is hashed with SHA-256, scoped to this workspace, rate-limited by plan,
            revocable in one click. Shown once at creation.
          </p>
        </div>
      </header>
      ...
      <section className="mt-10">
        <h2 className="font-mono text-[11px] uppercase tracking-[0.18em] text-muted-foreground/80">
          Your keys
        </h2>
        {isLoading ? (
          ...
          <ul className="mt-4 divide-y divide-border/40">
            ...
            {revoked.length > 0 && (
              <>
                <li className="pt-6 font-mono text-[10.5px] uppercase tracking-[0.18em] text-muted-foreground/60">
                  Revoked
                </li>
                {revoked.map((k) => (
          ...
          <div className="mx-auto flex max-w-5xl flex-wrap items-center gap-4 px-6 py-6">
            <div className="flex-1">
              <p className="font-mono text-[10.5px] uppercase tracking-[0.18em] text-primary">
                Save this key — one-time reveal
              </p>
              <Code code={createdKey} lang="text" className="mt-2 text-[13px]" />
            </div>
            <button
              onClick={async () => {
...
function KeyRow({ k, onRevoke, muted }: { k: ApiKeyRow; onRevoke?: () => void; muted?: boolean }) {
  return (
