...
import { ArrowRight, Layers, Wand2, Sparkles, Users, Image as ImageIcon, Boxes, Database, Braces, ShieldCheck } from "lucide-react";
...
export const Route = createFileRoute("/product")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "The DataSeed engine: multi-format parsing, coherent personas, rich types, and asset generation." },
  ]}),
    ...
    <PublicLayout>
        ...
        <div className="mx-auto max-w-7xl px-6 py-24">
          <div className="grid items-center gap-14 lg:grid-cols-2">
            <div className="illu-halo order-2 lg:order-1">
              <BlendedImage src={parsers} alt="Multi-format schema parsers" width={1600} height={1024} fade="all" glow={false} />
            </div>
            <div className="order-1 lg:order-2">
              <SectionHead align="left" eyebrow="Parsing" title="Every format your team already uses."
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-2">
                <FeatureCard icon={<Database className="h-5 w-5" />} title="SQL DDL" tags={["Postgres","MySQL","SQLite"]}>
                  Full <code>CREATE TABLE</code> parsing including FKs, enums, CHECK, NOT NULL, defaults.
                </FeatureCard>
                <FeatureCard icon={<Layers className="h-5 w-5" />} title="Prisma & Drizzle" tags={["schema.prisma","pgTable"]}>
                  ORM models parsed natively. Relations become topologically ordered inserts.
                </FeatureCard>
                <FeatureCard icon={<Braces className="h-5 w-5" />} title="Zod & TypeScript" tags={["z.object","runtime"]}>
                  Runtime Zod schemas parsed, refined, and mapped to realistic generators.
                </FeatureCard>
                <FeatureCard icon={<Boxes className="h-5 w-5" />} title="OpenAPI 3" tags={["JSON Schema","YAML"]}>
      ...
      <section className="border-b border-border/40 bg-[oklch(0.13_0.012_250)]/40">
        <div className="mx-auto max-w-7xl px-6 py-24">
          <div className="grid items-center gap-14 lg:grid-cols-2">
            <div>
              <SectionHead align="left" eyebrow="Coherence" title="A row is a person, not a random cell."
            ...
            <div className="illu-halo">
              <BlendedImage src={personas} alt="Coherent personas" width={1600} height={1024} fade="all" glow={false} />
            </div>
          </div>

          <ScrollReveal>
            <div className="mx-auto mt-16 max-w-3xl">
              <TerminalCard title="persona.json" badge="OUTPUT" accent>{SAMPLE}</TerminalCard>
            </div>
          </ScrollReveal>
        </div>
      </section>
      ...
      <section className="border-b border-border/40">
        <div className="mx-auto max-w-7xl px-6 py-24">
          <div className="grid items-center gap-14 lg:grid-cols-2">
            <div className="illu-halo order-2 lg:order-1">
              <BlendedImage src={types} alt="Rich Postgres types" width={1600} height={1024} fade="all" glow={false} />
            </div>
            <div className="order-1 lg:order-2">
              <SectionHead align="left" eyebrow="Rich types" title="Real Postgres. All of it."
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-2">
                <FeatureCard icon={<Wand2 className="h-5 w-5" />} title="pgvector" tags={["vector(1536)"]}>
                  Deterministic vectors, uniform or clustered, with configurable dimensions.
                </FeatureCard>
                <FeatureCard icon={<Wand2 className="h-5 w-5" />} title="PostGIS" tags={["geo-point","polygon"]}>
                  Lat/lng and polygons within a locale's realistic bounding box.
                </FeatureCard>
                <FeatureCard icon={<Wand2 className="h-5 w-5" />} title="JSONB & arrays" tags={["jsonb","text[]"]}>
                  Structured nested payloads matching your inferred domain.
                </FeatureCard>
                <FeatureCard icon={<Wand2 className="h-5 w-5" />} title="Bytea, inet, interval" tags={["bytea","inet"]}>
      ...
      <section className="border-b border-border/40 bg-[oklch(0.13_0.012_250)]/40">
        <div className="mx-auto max-w-7xl px-6 py-24">
          <div className="grid items-center gap-14 lg:grid-cols-2">
            <div>
              <SectionHead align="left" eyebrow="Assets" title="Faces, product shots, banners — sourced automatically."
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-2">
                <FeatureCard icon={<Users className="h-5 w-5" />} title="Avatars" tags={["DiceBear","Pravatar"]}>
                  Personalized to each persona's name, gender, and locale.
                </FeatureCard>
                <FeatureCard icon={<ImageIcon className="h-5 w-5" />} title="Product & cover images" tags={["Unsplash","Picsum"]}>
                  Topic-aware images derived from your table names and column hints.
                </FeatureCard>
              </div>
            </div>
            <div className="illu-halo">
              <BlendedImage src={assets} alt="Automatic asset generation" width={1600} height={1024} fade="all" glow={false} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="mx-auto max-w-4xl px-6 py-24 text-center">
          ...
          <h2 className="mt-5 text-balance text-3xl font-semibold tracking-tight sm:text-4xl">
            Deterministic. Reproducible. <span className="font-display italic text-primary">Yours.</span>
          </h2>
          <p className="mx-auto mt-4 max-w-xl text-muted-foreground">
            Same schema, same seed, same output — bit-for-bit. Your data never leaves the request/response cycle.
          </p>
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Button asChild size="lg" className="shadow-glow"><Link to="/signup">Create a free account</Link></Button>
